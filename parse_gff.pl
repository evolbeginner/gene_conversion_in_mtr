...
foreach my $indir (glob "$dir/gff/*"){
	...
	$ref_gff = &extract_gff(\@infile,$taxa_uid);
	...
	foreach my $num (0..$#ordered_gff_info){
		...
		@jie_qu_updown_stream_gene_num = reverse @jie_qu_updown_stream_gene_num if $ref_gff->{$key1}{'strand'} eq '-';
		foreach (@jie_qu_updown_stream_gene_num){
			...
			foreach (@print_out){
				...
				if ($XiangDui_strand_swi){
					...
					print $ref_gff->{$key}{$_}." $ref_gff->{$key}{'protein_id'} $XiangDui_strand"."\t";
				}
				else{
					print $ref_gff->{$key}{$_}."\t";					
				}
...
foreach my $infile(@$ref){
	...
	while(my $line=<$IN_handle>){
		...
		$gff{$return_key}{'COG'}=$COG;
		$gff{$return_key}{'product'}=$product;
		$gff{$return_key}{'protein_id'}=$protein_id;
		$gff{$return_key}{'start'}=$start;
		$gff{$return_key}{'end'}=$end;
		$gff{$return_key}{'strand'}=$strand;
		$gff{$return_key}{'info'}=$info;
		}
	}
	close IN;
}
